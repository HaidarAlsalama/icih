...
import sitemap from "../../../configs/siteMap.json";
export default function BreadcrumbContext() {
  ...
  return (
    <div className={" md:mb-2 flex flex-wrap gap-2"}>
      {locationLine.length > 0
        ? locationLine.map((item, index) => (
            <div className="flex items-center" key={index}>
              ...
              <Link
                to={item.url}
                className="text-sm  text-nowrap font-bold text-gray-500 hover:text-red-600 dark:text-gray-400 hover:dark:text-blue-600 duration-700 "
              >
                {t(item.title)}
              </Link>
            </div>
