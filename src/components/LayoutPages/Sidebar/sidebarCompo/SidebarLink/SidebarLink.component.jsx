...
export default function SidebarLink({ item }) {
  ...
  return (
    <li>
      ...
      >
        <div
          className={`hidden relative w-full md:flex items-center gap-2 md:flex-row-reverse lg:flex-row navbarStat_e overflow-hidden`}
        >
          <div className="p-2 ">
            <item.icon />
          </div>
          ...
          {item.count && item.count > 0 ? (
            <div className="absolute inline-flex items-center p-[10px] justify-center w-5 h-5 text-xs font-bold text-white bg-red-600 border-2 border-white rounded-full  end-1 dark:border-gray-900  dark:bg-blue-600 counter">
              {item.count}
            </div>
          ) : null}
        </div>
