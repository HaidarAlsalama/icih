...
import imageDemo from "./../../../images/eara.png";
import { api_host, api_image } from "../../../configs";
...
import SubscriptionAppInformation from "../../SubscriptionAppInformation/SubscriptionAppInformation";
export default function EditAndUploadImageFromClient({
  info,
  rand,
  isOverWrite = false,
  overWrite,
  ...
  const handleSendMessage = async () => {
    ...
    setOnSend(true);
    console.log(userAuth.token);
    const data = { message: sendMessageOk };
    await axios
      .patch(
        `${api_host}/subscriptions/update-state-check/${paperInfo.id}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${userAuth.token}`,
